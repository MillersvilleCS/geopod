		...
		extends ShadowType
{
	...
	/**
	 * Do we try to merge Geometries into existing Shape3D scene graph
	 * components
	 */
	...
	{
		if (Parent != null && Parent.inheritedText != null && Parent.inheritedTextControl != null)
		{
	 ...
	 */
	@Override
	public int checkIndices (int[] indices, int[] display_indices, int[] value_indices, boolean[] isTransform,
			int levelOfDifficulty)
			throws VisADException, RemoteException
	{
		return adaptedShadowType.checkIndices (indices, display_indices, value_indices, isTransform, levelOfDifficulty);
	}
	 ...
	 * values; default_values are defaults for each
	 * display.DisplayRealTypeVector; return true if need post-process; this is
	 * default (for ShadowTextType)
	 * 
	 * @param group
	 *            group to add to
	 * @param data
	 ...
	 * @throws VisADException
	 */
	public boolean doTransform (Object group, Data data, float[] value_array, float[] default_values,
	...
	/**
	 * render accumulated Vector of value_array-s to and add to group; then
	 * clear AccumulationVector
	 * 
	 * @param group
	 * 
	 ...
	 * @throws VisADException
	 */
	public static void mapValues (float[][] display_values, double[][] values, ShadowRealType[] reals)
			throws VisADException
	{
	 ...
	 * @throws VisADException
	 */
	public static void mapValues (float[][] display_values, float[][] values, ShadowRealType[] reals)
			throws VisADException
	{
	 ...
	 * @throws VisADException
	 */
	public static void mapValues (float[][] display_values, float[][] values, ShadowRealType[] reals, boolean copy)
			throws VisADException
	{
	 ...
	 * @throws VisADException
	 */
	public static VisADGeometryArray makePointGeometry (float[][] spatial_values, byte[][] color_values)
			throws VisADException
	{
	...
	{
		return ShadowTypeJ3D.staticMakeCachedAppearance (mode, constant_alpha, constant_color, geometry, no_material,
				false);
	}
	 ...
	 * and contains geometries of the same type. If found it will add the
	 * geometry to that Shape3D node. If not found or if mergeShapes is false
	 * then this behaves normally, creating a new Shape3D node and adding it to
	 * the group.
	 * 
	 * @param group
	 *            group to add to
	...
	/**
	 * A utility method to create a ColoringAttribute with the given colors If
	 * the cacheAppearances flag is true then this will try to find the
	...
	private ColoringAttributes getColoringAttributes (float red, float green, float blue)
	{
		...
		if (ca == null)
		{
			ca = new ColoringAttributes (red, green, blue, ColoringAttributes.SHADE_GOURAUD);
	...
	/**
	 * A utility method to create a TransparencyAttributes with the given mode
	 * and value. If the flag cacheAppearances is true then this will try to
	 * find the TransparencyAttributes in the cache. If false it just creates a
	 * new one. Note: If caching is on then the result object should be treated
	 * as being immutable
	 * 
	 * @param mode
	 *            Transparency mode
	 ...
	 * @param no_material
	 *            true to not use a Material for illumination, false to use it
	 *            for 2-D geometries
	 * 
	 * @return The new appearance
	 */
	public static Appearance staticMakeAppearance (GraphicsModeControl mode, TransparencyAttributes constant_alpha,
			ColoringAttributes constant_color, GeometryArray geometry, boolean no_material)
	{

		return staticMakeCachedAppearance (mode, constant_alpha, constant_color, geometry, no_material, false);
	}
			...
			boolean no_material, boolean okToCache)
	{
		...
		if (!(geometry instanceof LineArray || geometry instanceof PointArray || geometry instanceof IndexedLineArray
				|| geometry instanceof IndexedPointArray || geometry instanceof IndexedLineStripArray || geometry instanceof LineStripArray))
		{
			if (!no_material)
			{
				doMaterial = true;
		...
		try
		{
			java.lang.reflect.Method method = polygon.getClass ().getMethod ("setPolygonOffsetFactor",
	...
	*/
	...
	@Override
	public VisADGeometryArray[] assembleShape (float[][] display_values, int valueArrayLength, int[] valueToMap,
			Vector MapVector, int[] valueToScalar, DisplayImpl display, float[] default_values, int[] inherited_values,
			float[][] spatial_values, byte[][] color_values, boolean[][] range_select, int index, ShadowType shadow_api)
			throws VisADException, RemoteException
	{
		return adaptedShadowType.assembleShape (display_values, valueArrayLength, valueToMap, MapVector, valueToScalar,
				display, default_values, inherited_values, spatial_values, color_values, range_select, index,
				shadow_api);
	}
	 ...
	 * flow2_values for any coordinate transform; if needed, return a spatial
	 * Set from spatial_values, with the same topology as domain_set (or an
	 ...
	 */
	@Override
	public Set assembleSpatial (float[][] spatial_values, float[][] display_values, int valueArrayLength,
			int[] valueToScalar, DisplayImpl display, float[] default_values, int[] inherited_values, Set domain_set,
			boolean allSpatial, boolean set_for_shape, int[] spatialDimensions, boolean[][] range_select,
			float[][] flow1_values, float[][] flow2_values, float[] flowScale, boolean[] swap, DataRenderer renderer,
			ShadowType shadow_api)
			throws VisADException, RemoteException
	{
		return adaptedShadowType.assembleSpatial (spatial_values, display_values, valueArrayLength, valueToScalar,
				display, default_values, inherited_values, domain_set, allSpatial, set_for_shape, spatialDimensions,
				range_select, flow1_values, flow2_values, flowScale, swap, renderer, shadow_api);
	}
	 ...
	 */
	@Override
	public void assembleFlow (float[][] flow1_values, float[][] flow2_values, float[] flowScale,
			float[][] display_values, int valueArrayLength, int[] valueToScalar, DisplayImpl display,
			float[] default_values, boolean[][] range_select, DataRenderer renderer, ShadowType shadow_api)
			throws VisADException, RemoteException
	{
		adaptedShadowType.assembleFlow (flow1_values, flow2_values, flowScale, display_values, valueArrayLength,
				valueToScalar, display, default_values, range_select, renderer, shadow_api);
	}
	 ...
	 */
	@Override
	public VisADGeometryArray[] makeFlow (int which, float[][] flow_values, float flowScale, float[][] spatial_values,
			byte[][] color_values, boolean[][] range_select)
			throws VisADException
	{
		return adaptedShadowType.makeFlow (which, flow_values, flowScale, spatial_values, color_values, range_select);
	}
	...
	@SuppressWarnings("rawtypes")
	@Override
	public VisADGeometryArray[] makeStreamline (int which, float[][] flow_values, float flowScale,
			float[][] spatial_values, Set spatial_set, int spatialManifoldDimension, byte[][] color_values,
			boolean[][] range_select, int valueArrayLength, int[] valueToMap, Vector MapVector)
			throws VisADException
	{
		return adaptedShadowType.makeStreamline (which, flow_values, flowScale, spatial_values, spatial_set,
				spatialManifoldDimension, color_values, range_select, valueArrayLength, valueToMap, MapVector);
	}
	...
	@SuppressWarnings("rawtypes")
	@Override
	public boolean makeContour (int valueArrayLength, int[] valueToScalar, float[][] display_values,
			int[] inherited_values, Vector MapVector, int[] valueToMap, int domain_length, boolean[][] range_select,
			int spatialManifoldDimension, Set spatial_set, byte[][] color_values, boolean indexed, Object group,
			GraphicsModeControl mode, boolean[] swap, float constant_alpha, float[] constant_color,
			...
			throws VisADException
	{
		return adaptedShadowType.makeContour (valueArrayLength, valueToScalar, display_values, inherited_values,
				MapVector, valueToMap, domain_length, range_select, spatialManifoldDimension, spatial_set,
				color_values, indexed, group, mode, swap, constant_alpha, constant_color, shadow_api, Domain,
	...
	@Override
	public void addLabelsToGroup (Object group, VisADGeometryArray[] arrays, GraphicsModeControl mode,
			...
			throws VisADException
	{
		...
		synchronized (MUTEX)
		{
			...
			if (!control.contourFilled () && arrays != null)
			{
				...
				for (int ii = 0; ii < n_labels; ii++)
				{
					...
					if (control.getAutoSizeLabels ())
					{
						...
						lbl_trans = new LabelTransform (segR_trans_group, p_cntrl, new VisADGeometryArray[] {
								array.expSegRight, array.segRightAnchor }, array.segRightScaleInfo, 1);
					...
					if (array.isStyled)
					{
						addToGroup (segL_trans_group, array.expSegLeft, styledMode, constant_alpha, constant_color);
						addToGroup (segR_trans_group, array.expSegRight, styledMode, constant_alpha, constant_color);
					}
					else
					{
						addToGroup (segL_trans_group, array.expSegLeft, mode, constant_alpha, constant_color);
						addToGroup (segR_trans_group, array.expSegRight, mode, constant_alpha, constant_color);
					}
				}
			...
			for (int ii = 0; ii < n_labels; ii++)
			{
				...
				if (control.getAutoSizeLabels ())
				{
					LabelTransform lbl_trans = new LabelTransform (lbl_trans_group, p_cntrl, new VisADGeometryArray[] {
							array.label, array.labelAnchor }, null, 0);
					projListener.LT_array[cnt][0][ii] = lbl_trans;
				}

				((Group) group).addChild (lbl_trans_group);

				addToGroup (lbl_trans_group, array.label, mode, constant_alpha, constant_color);
			}
			cnt++;
	 ...
	 */
	@Override
	public VisADGeometryArray makeText (String[] text_values, TextControl text_control, float[][] spatial_values,
			byte[][] color_values, boolean[][] range_select)
			throws VisADException
	{
		return adaptedShadowType.makeText (text_values, text_control, spatial_values, color_values, range_select);
	}
	 ...
	 */
	@Override
	public byte[][] assembleColor (float[][] display_values, int valueArrayLength, int[] valueToScalar,
			DisplayImpl display, float[] default_values, boolean[][] range_select, boolean[] single_missing,
			ShadowType shadow_api)
			throws VisADException, RemoteException
	{
		return adaptedShadowType.assembleColor (display_values, valueArrayLength, valueToScalar, display,
				default_values, range_select, single_missing, shadow_api);
	}
	 ...
	 */
	@Override
	public boolean[][] assembleSelect (float[][] display_values, int domain_length, int valueArrayLength,
			int[] valueToScalar, DisplayImpl display, ShadowType shadow_api)
			throws VisADException
	{
		return adaptedShadowType.assembleSelect (display_values, domain_length, valueArrayLength, valueToScalar,
				display, shadow_api);
	}
	 ...
	 * @throws VisADException
	 */
	public boolean terminalTupleOrScalar (Object group, float[][] display_values, String text_value,
			TextControl text_control, int valueArrayLength, int[] valueToScalar, float[] default_values,
			int[] inherited_values, DataRenderer renderer)
			throws VisADException, RemoteException
	{

		boolean post = adaptedShadowType.terminalTupleOrScalar (group, display_values, text_value, text_control,
				valueArrayLength, valueToScalar, default_values, inherited_values, renderer, this);
	...
	public static void ensureNotEmpty (Object obj, DisplayImpl display)
	{
		...
		if (group.numChildren () > 0)
			return;
		GeometryArray geometry = new PointArray (1, GeometryArray.COORDINATES | GeometryArray.COLOR_3);
		...
		Appearance appearance = staticMakeCachedAppearance (display.getGraphicsModeControl (), null, null, geometry,
				false, true);
	 ...
	 */
	@Override
	public boolean addToGroup (Object group, VisADGeometryArray array, GraphicsModeControl mode, float constant_alpha,
			...
			throws VisADException
	{
		...
		if (array != null && array.vertexCount > 0)
		{
			...
			{
				c_alpha = getTransparencyAttributes (mode.getTransparencyMode (), constant_alpha);
				af = constant_alpha;
			}
			else
			{
			...
			if (constant_color != null && constant_color.length == 3)
			{
				...
				// constant_color[2]);
				c_color = getColoringAttributes (constant_color[0], constant_color[1], constant_color[2]);

				// WLH 16 Oct 2001
				if (!(array instanceof VisADLineArray || array instanceof VisADPointArray || array instanceof VisADLineStripArray)
						&& array.colors == null)
				{
					int color_len = 3;
					if (af != 0.0f)
	 ...
	 */
	@Override
	public boolean addTextToGroup (Object group, VisADGeometryArray array, GraphicsModeControl mode,
			...
			throws VisADException
	{
		if (array != null && array.vertexCount > 0)
		{
			...
			if (array instanceof VisADTriangleArray)
			{
				...
				Appearance appearance2 = makeCachedAppearance (mode, c_alpha, c_color, geometry2, false, false);
...
class LabelTransform
{
	 ...
	 * @param flag
	 */
	LabelTransform (TransformGroup trans, ProjectionControl proj, VisADGeometryArray[] label_array, float[] f_array,
			int flag)
	{
		this.trans = trans;
		this.proj = proj;
	...
	public void controlChanged (double first_scale, double[] scale_a)
	{
		...
		{ // -- expanding line segments
			double k = (f_array[0] / (f_array[1] - f_array[0])) * (f_array[1] / f_array[0] - first_scale / scale_a[0])
					* scale_a[0];
			factor = k / scale_a[0];
			f_scale = (float) ((scale_a[0] - k) / scale_a[0]);
		}
