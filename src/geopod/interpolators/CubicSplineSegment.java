...
/**
 * The CubicSplineSegment class creates the representation of a TCB
 * (Kochanek-Bartels Spline). This class takes 4 key frames as its input (using
 * TCBKeyFrame). If interpolating between the i<sup>th</sup> and
 * (i+1)<sup>th</sup> key frame then the four key frames that need to be
 * specified are the (i-1)<sup>th</sup>, i<sup>th</sup>, (i+1)<sup>th</sup> and
 * (i+2)<sup>th</sup> keyframes in order. The CubicSegmentClass then
 * pre-computes the hermite interpolation basis coefficients if the
 ...
 * (i+1)<sup>th</sup> frame's linear flag is set to 1 then the class uses linear
 * interpolation to calculate the interpolated position, sccale and quaternions
 * it returns through the getInterpolated* methods.
 * 
 * @since Java3D 1.1
 */
...
public class CubicSplineSegment
{
	...
	static final double modRoot[] = { 0.046910077, 0.230765345, 0.5, 0.769234655, 0.953089922 };

	// original coefficients divided by 2
	static final double modCoeff[] = { 0.118463442, 0.239314335, 0.284444444, 0.239314335, 0.118463442 };
	...
	/**
	 * Creates a cubic spline segment between two key frames using the key
	 * frames provided. If creating a spline between the ith frame and the
	 * (i+1)<sup>th</sup> frame then send down the (i - 1)<sup>th</sup>,
	 * i<sup>th</sup> , (i+1)<sup>th</sup> and the (i+2)<sup>th</sup> key
	 * frames.
	 * 
	 * @param kf0
	 *            (i - 1)<sup>th</sup> Key Frame
	 * @param kf1
	 *            i<sup>th</sup> Key Frame
	 * @param kf2
	 *            (i + 1)<sup>th</sup> Key Frame
	 * @param kf3
	 *            (i + 2)<sup>th</sup> Key Frame
	 */
	...
	/**
	 * Computes the interpolated quaternion along the curve at a given point
	 * between key frames. This routine uses linear interpolation if the
	 * (i+1)<sup>th</sup> key frame's linear value is equal to 1.
	 * 
	 * @param u
	 *            specifies the point between keyframes where 0 <= u <= 1.
	 * @param newQuat
	 *            returns the value of the interpolated quaternion
	 */
	...
	public void getInterpolatedQuaternion (float u, Quat4f newQuat)
	{
		...
		if (this.linear == 1)
		{
			...
			quatDot = keyFrame[1].quat.x * keyFrame[2].quat.x + keyFrame[1].quat.y * keyFrame[2].quat.y
					+ keyFrame[1].quat.z * keyFrame[2].quat.z + keyFrame[1].quat.w * keyFrame[2].quat.w;
		...
		else
		{
			...
			// TODO GZ: Currently we just use the great circle spherical interpolation
	...
	/**
	 * Computes the interpolated scale along the curve at a given point between
	 * key frames and returns a Point3f with the interpolated x, y, and z scale
	 * components. This routine uses linear interpolation if the
	 * (i+1)<sup>th</sup> key frame's linear value is equal to 1.
	 * 
	 * @param u
	 *            specifies the point between keyframes where 0 <= u <= 1.
	 ...
	 * between key frames and returns a Point3f with the interpolated x, y, and
	 * z scale components. This routine uses linear interpolation if the
	 * (i+1)<sup>th</sup> key frame's linear value is equal to 1.
	 * 
	 * @param u
	 *            specifies the point between keyframes where 0 <= u <= 1.
	...
	/**
	 * Computes the interpolated position along the curve at a given point
	 * between key frames and returns a Vector3f with the interpolated x, y, and
	 * z scale components. This routine uses linear interpolation if the
	 * (i+1)<sup>th</sup> key frame's linear value is equal to 1.
	 * 
	 * @param u
