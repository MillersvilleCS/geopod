...
import geopod.gui.panels.moviecapture.MovieCapturePanel;
		...
		extends JFrame
{
	...
	private OffScreenCanvas3D m_OffScreenTopCanvas;
	...
	private void addLayeredComponent (Component c, Integer layer, Rectangle constraints)
	{
	...
	{
		m_dashboardPanel = new ImagePanel ("//Resources/Images/User Interface/GeopodDashboardNoDisplay.png");
	...
	private void buildInterfaceComponents ()
	{
		...
		m_OffScreenCanvas = createOffScreenCanvas (CANVAS_SIZE);
		m_canvas.setOffScreenCanvas (m_OffScreenCanvas);

		addLayeredComponent (m_canvas, JLayeredPane.DEFAULT_LAYER, INNER_VIEWING_AREA_BOUNDS);
		addLayeredComponent (m_OffScreenCanvas, JLayeredPane.DEFAULT_LAYER, INNER_VIEWING_AREA_BOUNDS);

		// Mini map
		JPanel topViewPanel = new JPanel ();
		topViewPanel.setLayout (new BorderLayout ());
		topViewPanel.setBorder (BorderFactory.createBevelBorder (BevelBorder.RAISED, Color.WHITE, Color.WHITE,
				Color.GRAY, Color.GRAY));

		m_OffScreenTopCanvas = createOffScreenCanvas (new Dimension (255, 255));
		...
		topViewPanel.add (m_topCanvas, BorderLayout.CENTER);
		addLayeredComponent (topViewPanel, JLayeredPane.PALETTE_LAYER, TOP_VIEW_CANVAS_BOUNDS);

		m_buttonPanel = new PrimaryButtonPanel (m_hud);
		addLayeredComponent (m_buttonPanel, JLayeredPane.PALETTE_LAYER, PRIMARY_BUTTON_PANEL_BOUNDS);

		JPanel toolPanel = new ToolPanel (m_hud);
		addLayeredComponent (toolPanel, JLayeredPane.PALETTE_LAYER, TOOL_PANEL_BOUNDS);

		NavigationPanelExt navPanelExt = new NavigationPanelExt (m_hud, m_geopod);
		addLayeredComponent (navPanelExt, JLayeredPane.PALETTE_LAYER, NAV_EXT_PANEL_BOUNDS);

		m_navigationPanel = new NavigationPanel (m_hud, m_geopod, m_canvas);
		addLayeredComponent (m_navigationPanel, JLayeredPane.PALETTE_LAYER, NAVIGATION_PANEL_BOUNDS);

		m_lookUpPanel = new LookUpPanel (m_navigationPanel);
		m_geopod.addObserver (m_lookUpPanel, GeopodEventId.GEOPOD_TRANSLATED);
		m_lookUpPanel.addObserver (m_hud, GeopodEventId.LOOKUP_BUTTON_STATE_CHANGED);
		addLayeredComponent (m_lookUpPanel, JLayeredPane.PALETTE_LAYER, LOOK_UP_PANEL_BOUNDS);

		GeopodButton helpButton = createHelpButton ();
		addLayeredComponent (helpButton, JLayeredPane.PALETTE_LAYER, HELP_BUTTON_BOUNDS);

		GeopodButton configButton = createConfigButton ();
		addLayeredComponent (configButton, JLayeredPane.PALETTE_LAYER, CONFIG_BUTTON_BOUNDS);

		GeopodButton muButton = createMuButton ();
		addLayeredComponent (muButton, JLayeredPane.PALETTE_LAYER, MU_BUTTON_BOUNDS);

		GeopodButton idvButton = createIdvButton ();
		addLayeredComponent (idvButton, JLayeredPane.PALETTE_LAYER, IDV_BUTTON_BOUNDS);

		ImagePanel nsfLogo = new ImagePanel (NSF_LOGO_PATH);
		nsfLogo.setOpaque (false);
		addLayeredComponent (nsfLogo, JLayeredPane.PALETTE_LAYER, NFS_LOGO_BOUNDS);
		...
		try
		{
			displayImage = FileLoadingUtility.loadBufferedImage (displayAreaImagePath);
		}
		catch (IOException e)
		{
			e.printStackTrace ();
		...
		addLayeredComponent (m_primaryDisplayPanel, JLayeredPane.PALETTE_LAYER, PRIMARY_DISPLAY_PANEL_BOUNDS);
		...
		m_secondaryDisplayPanel.addObserver (m_overFlowPanel, GeopodEventId.DISPLAY_PANEL_ACTIVE);
		m_secondaryDisplayPanel.addObserver (m_overFlowPanel, GeopodEventId.DISPLAY_PANEL_EMPTY);
		m_overFlowPanel.add (m_secondaryDisplayPanel, BorderLayout.CENTER);
		addLayeredComponent (m_overFlowPanel, JLayeredPane.PALETTE_LAYER, OVERFLOW_PANEL_BOUNDS);

		m_parameterChooserPanel = m_hud.createParameterChooserPanel ();
		m_parameterChooserPanel.addObserver (m_hud, GeopodEventId.PARAMETER_BUTTON_STATE_CHANGED);
		m_parameterChooserPanel.setBorder (geopod.gui.components.BorderFactory.createStandardBorder ());
		m_parameterChooserPanel.setVisible (false);
		addLayeredComponent (m_parameterChooserPanel, JLayeredPane.MODAL_LAYER, PARAMETER_CHOOSER_PANEL_BOUNDS);

		m_missionPanel = new MissionPanel (m_geopod.getFlightRecorder ());
		m_missionPanel.setVisible (false);
		m_missionPanel.addObserver (m_hud, GeopodEventId.MISSION_BUTTON_STATE_CHANGED);
		addLayeredComponent (m_missionPanel, JLayeredPane.MODAL_LAYER, MISSION_PANEL_BOUNDS);
		...
		m_isosurfaceViewPanel.addObserver (m_hud, GeopodEventId.ISOSURFACE_BUTTON_STATE_CHANGED);
		addLayeredComponent (m_isosurfaceViewPanel, JLayeredPane.MODAL_LAYER, ISOSURFACE_VIEW_PANEL_BOUNDS);
		...
		catch (SecurityException | IllegalArgumentException | AWTException e1)
		{
			// TODO Auto-generated catch block
			System.err.println ("ERROR: Failed to create MovieCapturePanel");
			e1.printStackTrace ();
		}
		...
		m_configurationPanel.addObserver (m_hud, GeopodEventId.CONFIG_BUTTON_STATE_CHANGED);
		addLayeredComponent (m_configurationPanel, JLayeredPane.POPUP_LAYER, CONFIG_PANEL_BOUNDS);
		...
		addLayeredComponent (m_helpPanel, JLayeredPane.POPUP_LAYER, HELP_PANEL_BOUNDS);

		m_multipleChartPanel = new MultipleDropsondeChartPanel (m_geopod.getDropsondeHistory (), m_hud);
		m_multipleChartPanel.addObserver (m_hud, GeopodEventId.DROPSONDE_BUTTON_STATE_CHANGED);
		addLayeredComponent (m_multipleChartPanel, JLayeredPane.PALETTE_LAYER, DROPSONDE_PANEL_BOUNDS);
		...
		particlePanel.setBorder (BorderFactory.createBevelBorder (BevelBorder.RAISED, Color.WHITE, Color.WHITE,
				Color.GRAY, Color.GRAY));
		particlePanel.setVisible (false);
		particlePanel.setBackground (Color.BLACK);

		addLayeredComponent (particlePanel, JLayeredPane.PALETTE_LAYER, PARTICLE_PANEL_BOUNDS);

		m_loadingPanel = new LoadingPanel ();
		m_loadingPanel.setVisible (false);
		addLayeredComponent (m_loadingPanel, JLayeredPane.POPUP_LAYER, LOADING_PANEL_BOUNDS);

		m_eventNotificationPanel = new NotificationPanel ();
		m_eventNotificationPanel.setVisible (false);
		addLayeredComponent (m_eventNotificationPanel, JLayeredPane.POPUP_LAYER, EVENT_NOTIFICATION_PANEL_BOUNDS);
		...
		m_notedLocationsPanel.addObserver (m_hud, GeopodEventId.NOTEPAD_BUTTON_STATE_CHANGED);
		addLayeredComponent (m_notedLocationsPanel, JLayeredPane.MODAL_LAYER, NOTED_LOCATIONS_PANEL_BOUNDS);

		m_commentPromptPanel = new CommentPromptPanel (m_geopod, m_notedLocationsPanel);
		m_commentPromptPanel.setVisible (false);
		m_commentPromptPanel.addObserver (m_hud, GeopodEventId.ADDNOTE_BUTTON_STATE_CHANGED);
		addLayeredComponent (m_commentPromptPanel, JLayeredPane.POPUP_LAYER, COMMENT_PROMPT_PANEL_BOUNDS);

		m_statusPanel = new StatusPanel ("test status message");
		m_statusPanel.setCollapsed (true);
		addLayeredComponent (m_statusPanel, JLayeredPane.PALETTE_LAYER, STATUS_PANEL_BOUNDS);
		...
		JSlider speedometer = new SpeedControlSlider (m_geopod.getSpeedometerModel ());
		...
		addLayeredComponent (speedometer, JLayeredPane.PALETTE_LAYER, new Rectangle (1628, 450, 30, 250));
	}

	private Canvas3D createCanvas (Dimension size)
	{
	...
	private GeopodButton createConfigButton ()
	{
		...
		GeopodButton configButton = new GeopodButton (imageUp, imageDown, imageHover);
		configButton.setToolTipText (" Settings ");
		configButton.setActionCommand ("settings");
		configButton.addActionListener (m_hud);
		m_hud.addObserver (configButton, GeopodEventId.CONFIG_BUTTON_STATE_CHANGED);
	...
	private GeopodButton createHelpButton ()
	{
		...
		GeopodButton helpButton = new GeopodButton (imageUp, imageDown, imageHover);
	...
	public void toggleParameterChooserPanel ()
	{
		...
		m_parameterChooserPanel.notifyObservers (GeopodEventId.PARAMETER_BUTTON_STATE_CHANGED);
	}
	...
	private class CanvasOnlyPolicy
