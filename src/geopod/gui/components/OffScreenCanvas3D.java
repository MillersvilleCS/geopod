		...
		extends Canvas3D
{
	...
	protected void postInit (OnScreenCanvas3D onScreenCanvas3D)
	{
		...
		this.setScreenCaptureSize (onScreenCanvas3D.getWidth (), onScreenCanvas3D.getHeight ());
	}

	protected void setContainer (BlockingDeque<BufferedImage> images)
	{
		m_images = images;
	}
	...
	protected void setScreenCaptureSize (int width, int height)
	{
		...
		BufferedImage bImage = new BufferedImage (m_screenCaptureWidth, m_screenCaptureHeight,
				BufferedImage.TYPE_INT_ARGB);
		m_imageBuffer = new ImageComponent2D (bImage.getType (), bImage);
		m_imageBuffer.setCapabilityIsFrequent (ImageComponent2D.ALLOW_IMAGE_READ);
	...
	{
		m_images.offer (m_imageBuffer.getImage ());
	}
