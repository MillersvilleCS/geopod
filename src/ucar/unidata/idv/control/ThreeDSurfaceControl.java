...
/**
 * A MetApps Display Control with Displayable and controls for one 3D isosurface
		...
		implements ISubject
{
	...
	/**
	 * Property for sharing transparency. Deprecated since transparencey is now
	 * done through color table sharing.
	 * 
	 * @deprecated
	 */
	...
	/**
	 * percentage of slider. We don't really use this, just keep it around for
	 * legacy bundles
	 */
	 ...
	 * the Displayable (empty of data thus far). This method is called from
	 * inside DisplayControlImpl.init(several args).
	 * 
	 * @param dataChoice
			...
			throws VisADException, RemoteException
	{
		...
		((GridDisplayable) myDisplay).setColoredByAnother (haveMultipleFields ());
	...
	private void makeInitialWidgets ()
	{
		...
		sliderComps = GuiUtils.makeSliderPopup (0, levelRange.getMaxInt (), levelRange.getMaxInt () / 2, listener);
		...
		levelReadout.addActionListener (new ActionListener ()
		{
			public void actionPerformed (ActionEvent ae)
			{
				...
				{
					userMessage ("Incorrect format: " + levelReadout.getText ());
				}
				catch (Exception exc)
				{
	...
	/**
	 * This gets called by the DisplayControlImpl.doMakeWidgetComponent (which
	 * is called by DisplayControlImpl.doMakeContents) to make the GUI contents
	 * of this Control, and allows this class to insert its own widgets as
	 * needed. Makes color table chooser and slider for surface value
	 * 
	 * @param controlWidgets
	 *            list of control widgets to populate
	 * 
	 * @throws RemoteException
	 *             Java RMI error
	 * @throws VisADException
			...
			throws VisADException, RemoteException
	{
		...
		try
		{
			if ((getDisplayUnit () != null) && Unit.canConvert (getDisplayUnit (), getDataUnit ()))
			{
	 ...
	 * 
	 * @param displayLevel
	 *            surface level in display units
	...
	/**
	 * Set the level value on the isosurface display and update all of the UI
	 * widgets.
	 * 
	 * @param rawLevel
	 *            new isourface value
	 ...
	 *            new isourface value
	 * @param andDoShare
	 *            If true then propagate this change.
	 * 
	 * @throws RemoteException
	 *             Java RMI error
	 * @throws VisADException
			...
			throws VisADException, RemoteException
	{
		...
		if ((rawDataUnit != null) && (getDisplayUnit () != null))
		{
			Real newVal = new Real (getDataRealType (), displayValue, getDisplayUnit ());
	 ...
	 *            type of sharing
	 * @param data
	 *            Array of data being shared. In this case, the first (and
	 *            only?) object in the array is the level
	 */
	...
	/**
	 * Reset the label that shows the level. The given displayValue is in terms
	 * of the display unit.
	 * 
	 * @param displayValue
	...
	/**
	 * Set the transparency property value, 0.0 to 1.0 only. Only for legacy
	 * bundles.
	 * 
	 * @param alpha
	 *            the transparency property value.
	 */
	...
	private void adjustTransparency (float newAlpha, boolean andDoShare)
	{
		try
		{
			...
			float[][] table = ColorTable.changeTransparency (myDisplay.getColorPalette (), alpha);
	...
	/**
	 * Set the initial state of the JSlider of isosurface value, with data
	 * limits, units, and labels.
	 * 
	 * @throws RemoteException
	 *             Java RMI error
	 * @throws VisADException
			...
			throws VisADException, RemoteException
	{
		...
		if (span > 50.0)
		{
			levelRange.set (dmin.getValue (rangeUnit), dmax.getValue (rangeUnit) + 0.5);
			// round min value to multiple of 5 or 2 (user request)
			if (levelRange.getAbsSpan () < 40)
			{
				levelRange.setMin ((double) (2 * (levelRange.getMinInt () / 2)));
			...
			levelSlider.setPaintTicks (true);
			levelSlider.setLabelTable (levelSlider.createStandardLabels (spacing));
		}
		else
		{ // case of a smaller range; use float labels
			float fmin = 0.0f, fmax = 100000.0f, fmid = 50000.0f;
			...
			for (int n = 0; n <= 4; n++)
			{
				labels.put (new Integer (levelRange.getMinInt () + n * ispace), new JLabel (getDisplayConventions ()
						.format (fmin + n * spacing)));
			}
	...
	protected void addDisplaySettings (DisplaySettingsDialog dsd)
	{
		...
		dsd.addPropertyValue (new Double (getSurfaceValue ()), "levelWithRawValue", "Isosurface Value",
