...
public class MovieCapturePanel extends JPanel implements ISubject, ActionListener, KeyListener {
	...
	/**
	 * For use with {@linkplain #m_chkMaintainAspectRatio maintaining aspect ratio}.<br>
	 * By keeping track of the dimension most recently changed, we can "smartly" disable one field while maintaining
	 * aspect ratio.
	 */
	 ...
	 * Contains the {@linkplain #m_fldOutputFrameRate field} that allows the user to specify the movie frame rate.
	 */
	 ...
	 * <p>
	 * Limits the chronological difference between images from different queues when any queue is producing faster than
	 * the others. This can happen when multiple {@linkplain #captureFrame() requests} for images are collapsed into a 
	 * single renderer call, which is possible in {@linkplain javax.swing.JComponent #paint(java.awt.Graphics) 
	 * JComponent.paint(Graphics g)} and {@linkplain javax.media.j3d.Canvas3D #renderOffScreenBuffer() 
	...
	/**
	 * Toggles the visibility of the movie capture settings window, 
	 * or if it is currently recording, stops the recording.
	 */
	public void click () {
	...
	private void initMainDialog () {
		...
		JPanel capturePanel = GuiUtils.center (GuiUtils.hflow (Misc.newList (m_btnCaptureImage, m_btnCaptureMovie), 25, 0));
		...
		JPanel captureSizePanel = GuiUtils.hflow (Misc.newList (m_lblCaptureWidth, m_fldCaptureWidth, m_lblCaptureHeight, m_fldCaptureHeight));
		...
		JPanel ratePanel = GuiUtils.hflow (Misc.newList (GuiUtils.rLabel (" Rate: "), m_fldCaptureRate, new JLabel (" frames/sec")));
		
		JPanel settingsPanel = GuiUtils.doLayout (new Component[] {captureSizePanel, m_chkMaintainAspectRatio, m_chkHideGeopodInterface, ratePanel}, 2, GuiUtils.WT_N, GuiUtils.WT_N);
		...
		JPanel framesPanel = GuiUtils.vbox (GuiUtils.left (m_lblPreviewFrameCount), GuiUtils.hflow (frameButtons, 17, 0));
		...
		imagesChanged ();
		GuiUtils.packDialog (m_dlgMain, GuiUtils.centerBottom (contents, GuiUtils.wrap (GuiUtils.inset (m_btnCloseMainDialog, 4))));
	}
	...
	private void initSaveDialog () {
		...
		if (saveDirectory.exists ()) {
			for (File oldImage : saveDirectory.listFiles ()) {
				if (!oldImage.delete ()) {
					System.err.println ("MovieCapturePanel.java failed to remove old image file: " + oldImage.getAbsolutePath ());
				}
			}
		} else {
		...
		m_pnlOutputFrameRate = LayoutUtil.left (GuiUtils.wrap (GuiUtils.hflow (Misc.newList (GuiUtils.rLabel (" Rate: "), m_fldOutputFrameRate, new JLabel (" frames/sec")))));
	...
	private void initPreviewDialog () {
		if (m_dlgPreview == null) {
			...
			m_btnImagePreviewPrevious = makeActionButton ("Previous Frame", CMD_PREVIEW_IMAGE_PREVIOUS, GuiUtils.getImageIcon (iconPath + "StepBack16.gif"));
			m_btnImagePreviewPlay 	  = makeActionButton ("Play", 			CMD_PREVIEW_IMAGE_PLAY, 	m_icnImagePreviewPlay);
			m_btnImagePreviewNext 	  = makeActionButton ("Next Frame", 	CMD_PREVIEW_IMAGE_NEXT, 	GuiUtils.getImageIcon (iconPath + "StepForward16.gif"));
			m_fldImagePreviewRate = new JTextField ("15.0", 3);
			List<JComponent> controlsList = Arrays.asList (m_btnImagePreviewPrevious, 
			                                               m_btnImagePreviewPlay, 
			                                               m_btnImagePreviewNext, 
			                                               GuiUtils.filler (20, 5), 
			                                               new JLabel (" Rate: "), 
			                                               m_fldImagePreviewRate, 
			...
			JComponent topComp = GuiUtils.leftRight (controlsPanel, GuiUtils.hbox (m_btnImagePreviewDelFrame, m_lblPreviewFrameNumber));
			...
			JComponent botComp = GuiUtils.wrap (LayoutUtil.center(LayoutUtil.hgrid (Misc.newList (m_btnImagePreviewSaveFrame, closePreviewBtn), 5)));
	...
	public void actionPerformed (ActionEvent ae) {
		...
		if (cmd.equals (CMD_CAPTURE_IMAGE)) {
			...
			try {
				showSaveImageDialog (ImageUtils.toBufferedImage (
                                         ImageUtils.getImageFile (m_images.get (m_previewIndex).getPath ())));
			} catch (Exception e) {
				e.printStackTrace ();
			}
	...
	/**
	 * <b>Warning:</b> The underlying calls made here can sometimes be collapsed into a single renderer call.
	 * <p>
	...
	/**
	 * Guarantees that temporary images on disk are loaded into {@code m_images} in correct chronological order.
	 */
	private void updatePreviewImageList () {
		...
		if (m_images.size () == 0) {
			...
			compareTimeStamp = Long.valueOf (comparePath.substring (comparePath.lastIndexOf (File.separatorChar) + 1, comparePath.length () - 4));
		}
	...
	private void showSaveMovieDialog () {
		...
		JpegImagesToMovie.createMovie (path, m_captureWidth, m_captureHeight, (int) (frameRate), new Vector (ImageWrapper.makeFileList (m_images)));
		...
		public void run () {
			...
			if (m_chkHideGeopodInterface.isSelected ()) {
				...
				if (m_unsavedMainImages.isEmpty () || m_unsavedMiniImages.isEmpty () || m_unsavedDashImages.isEmpty ()) {
					...
					BufferedImage frameImage = new BufferedImage (m_captureWidth, m_captureHeight, BufferedImage.TYPE_INT_ARGB);
	...
	private BufferedImage getDashboardImage () {
		BufferedImage dashImg = new BufferedImage (m_dashboard.getWidth (), m_dashboard.getHeight (), BufferedImage.TYPE_INT_ARGB);
	...
	/**
	 * Paints only the dashboard components required to figure out the main and mini canvases' bounding rectangles post-scale op.
	 */
	private BufferedImage getDashboardSkeletonImage (int width, int height) {
		BufferedImage dashImg = new BufferedImage (m_dashboard.getWidth (), m_dashboard.getHeight (), BufferedImage.TYPE_INT_ARGB);
		BufferedImage miniImg = new BufferedImage (m_miniPanel.getWidth (), m_miniPanel.getHeight (), BufferedImage.TYPE_INT_ARGB);
	...
	/**
	 * Starting from the center of the image, finds the offset and dimensions of a contiguous black area using an
	 * iterative pixel color comparison.
	 */
	private Rectangle findBlackArea (BufferedImage image) {
		...
		int[] pixelLineHorz = image.getRGB (0, image.getHeight () / 2, image.getWidth (), 1, null, 0, image.getWidth ());
	...
	/************************************************* Miscellany ****************************************************/
	/**
	 * Changes the <code>isEnabled</code> value of all buttons/text fields of <code>m_dlgMain</code>.
	 * <p>
	 * Set to false when movie capture starts and set to true when finished saving images.
	 * 
	 * @param enabled
	...
	private boolean updateCaptureSettings () {
		...
		if (captureRateDesired == 0.0 || captureWidth == 0 || captureHeight == 0) {
			JOptionPane.showMessageDialog (m_dlgMain, "Please enter a value greater than zero.", "Invalid Input", JOptionPane.ERROR_MESSAGE);
			return false;
		} else {
	...
	/** 
	 * Changes the text in the capture width/height fields when {@linkplain #m_chkMaintainAspectRatio maintaining
	 *  aspect ratio}.
	 */
