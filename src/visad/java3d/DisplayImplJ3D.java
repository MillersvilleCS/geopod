 ...
 * MEMORY LEAK
 * 
 * DisplayImplJ3D.destrory() DisplayRendererJ3D.destroy() VisADCanvasJ3D.stop()
 * stopRenderer() DisplayPanelJ3D.destroy() display, renderer = null;
 * universe.destroy() DisplayAppletJ3D.destroy() display, renderer = null;
 * universe.destroy() display, component = null MouseBehaviorJ3D.destroy()
 * helper, display, display_renderer = null root.detach() root, trans, vpTrans,
 * non_direct, view, canvas = null DisplayImpl.destroy() stop()
 * removeThingChangedListener() for all links LinkVector.removeAllElements()
 * pool.queue(this) run_links = null DisplayActivity.destroy() notify listeners
 * remove all listeners clearMaps() check RendererVector empty
 * MapVector.removeAllElements() ConstantMapVector.removeAllElements()
 * RealTypeVector.removeAllElements() AnimationControlJ3D.stop() animationThread
 * = null // causes run() to exit applet, projection, mode = null;
 */
...
/**
 * DisplayImplJ3D is the VisAD class for displays that use Java 3D. It is
 * runnable.
 * <P>
 * 
		...
		extends DisplayImpl
{
	 ...
	 */
	public static final int POLYGON_FILL = javax.media.j3d.PolygonAttributes.POLYGON_FILL;
	 ...
	 * @see GraphicsModeControlJ3D#setPolygonMode
	 */
	public static final int POLYGON_LINE = javax.media.j3d.PolygonAttributes.POLYGON_LINE;

	/**
	 * Render polygonal primitives as points drawn at the vertices of the
	 * polygon.
	 * 
	 * @see GraphicsModeControlJ3D#setPolygonMode
	 */
	public static final int POLYGON_POINT = javax.media.j3d.PolygonAttributes.POLYGON_POINT;
	 ...
	 */
	public static final int NICEST = javax.media.j3d.TransparencyAttributes.NICEST;
	 ...
	 * @see GraphicsModeControlJ3D#setTransparencyMode
	 */
	public static final int FASTEST = javax.media.j3d.TransparencyAttributes.FASTEST;
	...
	/**
	 * Field for specifying that the DisplayImpl does not have a screen
	 * Component
	 */
	...
	/**
	 * Indicates whether to use geometry by reference when creating geometry
	 * arrays.
	 * 
	 ...
	 */
	// FIXME:
	// This works with the Java3D 1.5.2 example TextureImageNPOT but does not
	// work
	// with the VisAD library image rednering. On initial testing it behaves as
	// if
	...
	/**
	 * construct a DisplayImpl for Java3D with the default DisplayRenderer, in a
	 * JFC JPanel
	 */
	...
	/**
	 * construct a DisplayImpl for Java3D with a non-default DisplayRenderer, in
	 * a JFC JPanel
	 */
	...
	/**
	 * construct a DisplayImpl for Java3D with a non-default DisplayRenderer; in
	 * a JFC JPanel if api == DisplayImplJ3D.JPANEL and in an AppletFrame if api
	...
	/**
	 * construct a DisplayImpl for Java3D with a non-default DisplayRenderer and
	 * GraphicsConfiguration, in a JFC JPanel
	 */
	public DisplayImplJ3D (String name, DisplayRendererJ3D renderer, GraphicsConfiguration config)
			throws VisADException, RemoteException
	{
	...
	public DisplayImplJ3D (String name, DisplayRendererJ3D renderer, int api, GraphicsConfiguration config)
			throws VisADException, RemoteException
	{
	...
	/**
	 * the 'c' argument is intended to be an extension class of VisADCanvasJ3D
	 * (or null); if it is non-null, then api must be JPANEL and its super()
	 * constructor for VisADCanvasJ3D must be 'super(renderer, config)'
	 */
	public DisplayImplJ3D (String name, DisplayRendererJ3D renderer, int api, GraphicsConfiguration config,
			VisADCanvasJ3D c)
			throws VisADException, RemoteException
	...
	/** constructor for off screen */
	public DisplayImplJ3D (String name, DisplayRendererJ3D renderer, int width, int height)
			throws VisADException, RemoteException
	{
	...
	/**
	 * constructor for off screen; the 'c' argument is intended to be an
	 * extension class of VisADCanvasJ3D (or null); if it is non-null, then its
	 * super() constructor for VisADCanvasJ3D must be 'super(renderer, width,
	 * height)'
	 */
	public DisplayImplJ3D (String name, DisplayRendererJ3D renderer, int width, int height, VisADCanvasJ3D c)
			throws VisADException, RemoteException
	{
		super (name, renderer);
	...
	public DisplayImplJ3D (RemoteDisplay rmtDpy, DisplayRendererJ3D renderer, int api)
			throws VisADException, RemoteException
	{
		this (rmtDpy, renderer, api, null);
	}

	public DisplayImplJ3D (RemoteDisplay rmtDpy, DisplayRendererJ3D renderer, GraphicsConfiguration config)
			throws VisADException, RemoteException
	{
		this (rmtDpy, renderer, rmtDpy.getDisplayAPI (), config);
	}

	public DisplayImplJ3D (RemoteDisplay rmtDpy, int api, GraphicsConfiguration config)
			throws VisADException, RemoteException
	{
		this (rmtDpy, null, api, config);
	}

	public DisplayImplJ3D (RemoteDisplay rmtDpy, DisplayRendererJ3D renderer, int api, GraphicsConfiguration config)
			throws VisADException, RemoteException
	{
	...
	{
		super (rmtDpy,
				((renderer == null && api == TRANSFORM_ONLY) ? new TransformOnlyDisplayRendererJ3D () : renderer));
	 ...
	 * 
	 * @return the mode being used (UNKNOWN, JPANEL, APPLETFRAME, OFFSCREEN,
	 *         TRANSFORM_ONLY)
			...
			throws VisADException
	{
		...
		if (vga instanceof VisADIndexedTriangleStripArray)
		{
			...
			if (vga.vertexCount == 0)
				return null;
			IndexedTriangleStripArray array = new IndexedTriangleStripArray (vga.vertexCount, vertexFormat,
			...
			/*
			 * this expands indices if (vga.vertexCount == 0) return null; // //
			 * expand vga.coordinates, vga.colors, vga.normals and vga.texCoords
			 * // int count = vga.indices.length; int len = 3 * count;
			 * 
			 * int sum = 0; for (int i=0; i<vga.stripVertexCounts.length; i++)
			 * sum += vga.stripVertexCounts[i];
			 * System.out.println("vga.indexCount = " + vga.indexCount +
			 * " sum = " + sum + " count = " + count +
			 * " vga.stripVertexCounts.length = " +
			 * vga.stripVertexCounts.length); // int[] strip_counts = new
			 * int[1]; // strip_counts[0] = count; // TriangleStripArray array =
			 * // new TriangleStripArray(count, makeFormat(vga), strip_counts);
			 * 
			 * TriangleStripArray array = new TriangleStripArray(count,
			 * makeFormat(vga), vga.stripVertexCounts);
			 * 
			 * if (vga.coordinates != null) {
			 * System.out.println("expand vga.coordinates"); float[] coords =
			 * new float[len]; for (int k=0; k<count; k++) { int i = 3 * k; int
			 * j = 3 * vga.indices[k]; coords[i] = vga.coordinates[j]; coords[i
			 * + 1] = vga.coordinates[j + 1]; coords[i + 2] = vga.coordinates[j
			 * + 2]; } array.setCoordinates(0, coords); } if (vga.colors !=
			 * null) { System.out.println("expand vga.colors"); byte[] cols =
			 * new float[len]; for (int k=0; k<count; k++) { int i = 3 * k; int
			 * j = 3 * vga.indices[k]; cols[i] = vga.colors[j]; cols[i + 1] =
			 * vga.colors[j + 1]; cols[i + 2] = vga.colors[j + 2]; }
			 * array.setColors(0, cols); } if (vga.normals != null) {
			 * System.out.println("expand vga.normals"); float[] norms = new
			 * float[len]; for (int k=0; k<count; k++) { int i = 3 * k; int j =
			 * 3 * vga.indices[k]; norms[i] = vga.normals[j]; norms[i + 1] =
			 * vga.normals[j + 1]; norms[i + 2] = vga.normals[j + 2]; }
			 * array.setNormals(0, norms); } if (vga.texCoords != null) {
			 * System.out.println("expand vga.texCoords"); float[] tex = new
			 * float[len]; for (int k=0; k<count; k++) { int i = 3 * k; int j =
			 * 3 * vga.indices[k]; tex[i] = vga.texCoords[j]; tex[i + 1] =
			 * vga.texCoords[j + 1]; tex[i + 2] = vga.texCoords[j + 2]; }
			 * array.setTextureCoordinates(0, tex); } return array;
			 */

			/*
			 * this draws normal vectors if (vga.vertexCount == 0) return null;
			 * LineArray array = new LineArray(2 * vga.vertexCount,
			 * LineArray.COORDINATES); float[] new_coords = new float[6 *
			 * vga.vertexCount]; int i = 0; int j = 0; for (int k=0;
			 * k<vga.vertexCount; k++) { new_coords[j] = vga.coordinates[i];
			 * new_coords[j+1] = vga.coordinates[i+1]; new_coords[j+2] =
			 * vga.coordinates[i+2]; j += 3; new_coords[j] = vga.coordinates[i]
			 * + 0.05f * vga.normals[i]; new_coords[j+1] = vga.coordinates[i+1]
			 * + 0.05f * vga.normals[i+1]; new_coords[j+2] =
			 * vga.coordinates[i+2] + 0.05f * vga.normals[i+2]; i += 3; j += 3;
			 * } array.setCoordinates(0, new_coords); return array;
			 */

			/*
			 * this draws the 'dots' if (vga.vertexCount == 0) return null;
			 * PointArray array = new PointArray(vga.vga.vertexCount,
			 * makeFormat(vga)); basicGeometry(vga, array, false); return array;
			 */
		}
	...
	private static int makeFormat (VisADGeometryArray vga)
	{
		...
		if (GEOMETRY_BY_REF)
			format |= GeometryArray.BY_REFERENCE;
		return format;
	}
