...
(function($){
	...
	bValidator = function(mainElement, overrideOptions, instanceName){
		...
		_getElementsForValidation = function(element){
			...
			// LMY : changed to not skip hidden elements, changed to validate divs not just inputs
			return element.is(':input,div') ? element : element.find(':input[' + options.validateActionsAttr + '],div[' + options.validateActionsAttr + ']').not(":button, :image, :reset, :submit, :disabled");
		},
						...
						if(errorMessages.length){
							...
							if (options.errorValidateOn){
								...
								else if ($(this).is('div'))
								{
									...
									$(this).bind('onclick.bVerror' + instanceName, {'bVInstance': instance}, function(event){
										event.data.bVInstance.validate(false, $(this));
									});
									
									var childCheckboxRadios = $(this).find(':checkbox,:radio');
									$(childCheckboxRadios).unbind('.bVerror' + instanceName);
									$(childCheckboxRadios).bind('change.bVerror' + instanceName, {'bVInstance': instance, 'parentDiv': $(this)}, function(event){
										event.data.bVInstance.validate(false, event.data.parentDiv);
									});
								}
								else{
									var evt = $(this).is('select,input:file') ? 'change' : options.errorValidateOn;
