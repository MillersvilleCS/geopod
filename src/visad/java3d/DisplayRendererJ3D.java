 ...
 * background and metadata rendering algorithms. These complement depictions of
 * <CODE>Data</CODE> objects created by <CODE>DataRenderer</CODE> objects.
 * <P>
 * 
 * <CODE>DisplayRendererJ3D</CODE> also manages the overall relation of
 * <CODE>DataRenderer</CODE> output to Java3D and manages the scene graph.
 * <P>
 * 
 * It creates the binding between <CODE>Control</CODE> objects and scene graph
 * <CODE>Behavior</CODE> objects for direct manipulation of <CODE>Control</CODE>
 * objects.
 * <P>
 * 
 * <CODE>DisplayRendererJ3D</CODE> is not <CODE>Serializable</CODE> and should
 * not be copied between JVMs.
 * <P>
 */
		...
		implements RendererSourceListener
{
	...
	/**
	 * single TransformGroup between root and BranchGroups for all Data
	 * depictions
	 */
	private TransformGroup trans = null;
	/**
	 * BranchGroup between trans and all direct manipulation Data depictions
	 */
	// WLH 13 March 2000
	...
	private Vector<AxisScale> axis_vector = new Vector<AxisScale> ();

	/** on / off state of cursor in GraphicsModeControl */

	/** Vector of DirectManipulationRenderers */
	private Vector<DirectManipulationRendererJ3D> directs = new Vector<DirectManipulationRendererJ3D> ();
	 ...
	 * 
	 * @param dpy
	 *            <CODE>Display</CODE> to render.
	 * @exception VisADException
	 *                If a <CODE>DisplayImpl</CODE> has already been specified.
	 */
	@Override
			...
			try
			{
				synchronized (this)
				{
					...
					if (canvas.getOffscreen ())
					{
						try
						{
							Method renderMethod = Canvas3D.class.getMethod ("renderOffScreenBuffer", new Class[] {});
	 ...
	 * <CODE>RendererControl</CODE> with current renderer settings before it is
	 * actually connected to the renderer. This means that changes will not
	 * generate <CODE>MonitorEvent</CODE>s.
	 * 
	 * @param ctl
	 *            RendererControl to initialize
	 */
	@Override
	 ...
	 * 
	 * @param evt
	 *            <CODE>ControlEvent</CODE> generated by a change to the
	...
	public void controlChanged (ControlEvent evt)
	{
		...
		if (box_color != null)
		{
			ct = ctl.getBoxColor ();
			box_color.getColor (c3f);
			if (!Util.isApproximatelyEqual (ct[0], c3f.x) || !Util.isApproximatelyEqual (ct[1], c3f.y)
	...
	/**
	 * 
	 * @return - A single TransformGroup between root and BranchGroups above all
	 *         Data depictions
	 */
	 ...
	 * 
	 * @param on
	 *            true to display the cursor, false to hide it.
	 */
	@Override
	 ...
	 * 
	 * @param on
	 *            true for enabling direct manipulation, false to disable
	 */
	@Override
	...
	*/
	...
	/**
	 * Create scene graph root, if none exists, with Transform and direct
	 * manipulation root; create special graphics (e.g., 3-D box, SkewT
	 * background), any lights, any user interface embedded in scene.
	 * 
	 * @param v
	 * @param vpt
	 * @param c
	 ...
	 */
	public BranchGroup createBasicSceneGraph (View v, TransformGroup vpt, VisADCanvasJ3D c, MouseBehaviorJ3D m,
			ColoringAttributes bc, ColoringAttributes cc)
	{
		if (root != null)
			return root;
		if (not_destroyed == null)
		...
		*/
		...
		try
		{
			...
			Constructor<javax.media.j3d.ModelClip> modelClipConstructor = modelClipClass.getConstructor (param);
			...
			int ALLOW_PLANE_WRITE = modelClipClass.getField ("ALLOW_PLANE_WRITE").getInt (modelClip);
			modelClipSetCapability.invoke (modelClip, new Object[] { new Integer (ALLOW_PLANE_WRITE) });
			int ALLOW_ENABLE_WRITE = modelClipClass.getField ("ALLOW_ENABLE_WRITE").getInt (modelClip);
			modelClipSetCapability.invoke (modelClip, new Object[] { new Integer (ALLOW_ENABLE_WRITE) });
	...
	// WLH 23 Oct 2001
	/**
	 * Define a clipping plane in (XAxis, YAxis, ZAxis) space. Allows up to 6
	 * arbitrary planes. Each clip plane is defined by the equation:
	 * 
	 * <PRE>
	 * aX + bY + cZ + d &lt;= 0
	 * </PRE>
	 ...
	 * Example useage:
	 * </p>
	 * To clip to the usual VisAD cube (i.e., x, y and z values in the range
	 * -1.0 to +1.0) (see Test35.java), call:
	 * 
	 * <PRE>
	 ...
	 * </PRE>
	 * 
	 * <b>Note:</b> d value is slightly less than -1.0 so items in the plane are
	 * not clipped.
	 * 
	 * @param plane
	 *            plane number must be in (0, ..., 5)).
	 * @param enable
	 *            true to enable clipping on this plane, false to disable
	 * @param a
	 *            x coefficent
	 * @param b
	 *            y coefficent
	 ...
	 *            constant
	 * @throws VisADException
	 *             illegal plane argument or unsupported (&lt; 1.2) version of
	 *             Java 3D
	 */
			...
			throws VisADException
	{
		...
		if (modelClip == null || modelClipSetEnable == null || modelClipSetPlane == null)
		{
			throw new DisplayException ("model clipping not supported in this " + "version of Java3D");
		}
		Vector4d vect = new Vector4d (a, b, c, d);
		try
		{
	...
	{
		if (not_destroyed == null || screen_locked == null)
			return;
	...
	public void drag_cursor (VisADRay ray, boolean first)
	{
		...
		*/
		...
		float dot = (cursorX - o_x) * line_x + (cursorY - o_y) * line_y + (cursorZ - o_z) * line_z;
	...
	/**
	 * Whenever <CODE>cursorOn</CODE> or <CODE>directOn</CODE> is true, display
	...
	public void drawCursorStringVector (VisADCanvasJ3D canvas)
	{
		...
		if (display != null && display.getGraphicsModeControl () != null)
		{
			...
			if (getDisplay ().getGraphicsModeControl ().getProjectionPolicy () == View.PERSPECTIVE_PROJECTION)
			{
				...
				Point3d eye = new Point3d ((left_eye.x + right_eye.x) / 2.0, (left_eye.y + right_eye.y) / 2.0,
		...
		double[] start = { position1.x, position1.y, position1.z };
		double[] base = { (position2.x - position1.x), (position2.y - position1.y), (position2.z - position1.z) };
		double[] up = { (position3.x - position1.x), (position3.y - position1.y), (position3.z - position1.z) };
		if (cursorOn || directOn)
		{
			Enumeration<String> strings = getCursorStringVector ().elements ();
		...
		double[] startl = { position3.x, -position3.y, position3.z };
		Vector<DataRenderer> rendererVector = getDisplay ().getRendererVector ();
		Enumeration<DataRenderer> renderers = rendererVector.elements ();
		while (renderers.hasMoreElements ())
		{
			DataRenderer renderer = renderers.nextElement ();
			Vector<Exception> exceptionVector = renderer.getExceptionVector ();
			Enumeration<Exception> exceptions = exceptionVector.elements ();
			while (exceptions.hasMoreElements ())
			{
				Exception error = exceptions.nextElement ();
		...
		if (animation_string[0] != null)
		{
			...
			if (nchars < 12)
				nchars = 12;
			double[] starta = { (-position2.x - nchars * (position2.x - position1.x)), -position3.y + 1.2 * up[1],
					position2.z };
			try
			{
		...
		if (scale_switch != null && scale_switch.getWhichChild () == 1)
		{
			...
			Enumeration<AxisScale> axes = axis_vector.elements ();
			while (axes.hasMoreElements ())
			{
				AxisScale axisScale = axes.nextElement ();
				try
				{
					boolean success = axisScale.makeScreenBasedScale (XMIN, YMIN, XMAX, YMAX, XTMIN, YTMIN, XTMAX,
							YTMAX);
					if (success)
					{
						...
						if (labels != null)
						{
							GeometryArray labelGeometry = ((DisplayImplJ3D) getDisplay ()).makeGeometry (labels);
							Appearance labelAppearance = ShadowTypeJ3D.staticMakeAppearance (getDisplay ()
									.getGraphicsModeControl (), null, null, labelGeometry, true);
	...
	/**
	 * Find the <CODE>DataRenderer</CODE> that is closest to the ray and uses
	 * the specified mouse modifiers for direct manipulation.
	 * 
	 * @param ray
	 *            position to check
	...
	public DataRenderer findDirect (VisADRay ray, int mouseModifiers)
	{
		...
		Enumeration<DirectManipulationRendererJ3D> renderers = ((Vector<DirectManipulationRendererJ3D>) directs
				.clone ()).elements ();
		while (renderers.hasMoreElements ())
		{
			DirectManipulationRendererJ3D r = renderers.nextElement ();
			if (r.getEnabled ())
			{
	...
	/**
	 * Check to see if there are any <CODE>DirectManipulationRenderer</CODE>s in
	 * this display.
	 * 
	 ...
	 * 
	 * @param on
	 *            turn on if true, otherwise turn them off
	 */
	@Override
			...
			throws VisADException
	{
		...
		{
			setScale (axisScale.getAxis (), axisScale.getAxisOrdinal (), axisScale.getScaleArray (),
					axisScale.getLabelArray (), axisScale.getColor ().getColorComponents (null));
		}
	}
	 ...
	 * 
	 * @param axis
	 *            axis for this scale (0 = XAxis, 1 = YAxis, 2 = ZAxis)
	 * @param axis_ordinal
	 *            position along the axis
	 * @param array
	 *            <CODE>VisADLineArray</CODE> representing the scale plot
	 * @param scale_color
	 *            array (dim 3) representing the red, green and blue color
	 *            values.
	 * @throws VisADException
	 *             couldn't set the scale
	 */
	@Override
	public void setScale (int axis, int axis_ordinal, VisADLineArray array, float[] scale_color)
			throws VisADException
	{
		if (not_destroyed == null)
			return;
	 ...
	 *            <CODE>VisADLineArray</CODE> representing the scale plot
	 * @param labels
	 *            <CODE>VisADTriangleArray</CODE> representing the labels
	 ...
	 */
	@Override
	public void setScale (int axis, int axis_ordinal, VisADLineArray array, VisADTriangleArray labels,
			float[] scale_color)
			throws VisADException
	{
		if (not_destroyed == null)
		...
		if (labels != null)
		{
			...
			if (labels instanceof VisADTriangleArray)
			{
				...
				Appearance labelAppearance2 = ShadowTypeJ3D.staticMakeAppearance (mode, null, null, labelGeometry2,
						true);
	...
	/**
	 * Factory for constructing a subclass of <CODE>Control</CODE> appropriate
	 * for the graphics API and for this <CODE>DisplayRenderer</CODE>; invoked
	 * by <CODE>ScalarMap</CODE> when it is <CODE>addMap()</CODE>ed to a
	 * <CODE>Display</CODE>.
	 * 
	 * @param map
	 *            The <CODE>ScalarMap</CODE> for which a <CODE>Control</CODE>
	 *            should be built.
	...
	public Control makeControl (ScalarMap map)
	{
		...
		if (type == null)
			return null;
		if (type.equals (Display.XAxis) || type.equals (Display.YAxis) || type.equals (Display.ZAxis)
				|| type.equals (Display.Latitude) || type.equals (Display.Longitude) || type.equals (Display.Radius))
		{
			return display.getProjectionControl ();
		}
		else if (type.equals (Display.RGB) || type.equals (Display.HSV) || type.equals (Display.CMY))
		{
		...
		else if (type.equals (Display.Flow1X) || type.equals (Display.Flow1Y) || type.equals (Display.Flow1Z)
				|| type.equals (Display.Flow1Elevation) || type.equals (Display.Flow1Azimuth)
		...
		else if (type.equals (Display.Flow2X) || type.equals (Display.Flow2Y) || type.equals (Display.Flow2Z)
				|| type.equals (Display.Flow2Elevation) || type.equals (Display.Flow2Azimuth)
	 ...
	 * 
	 * @param renderer
	 *            <CODE>DataRenderer</CODE> to check
